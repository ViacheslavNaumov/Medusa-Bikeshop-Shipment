...
echo "=== Starting Bikeshop Shipment Application in Production Mode ==="
echo ""
...
# Build the Medusa server
echo -e "${YELLOW}Building Medusa server...${NC}"
cd /workspace/Medusa-Bikeshop-Shipment/bikeshop-shipment
npm run build
if [ $? -eq 0 ]; then
  echo -e "  ${GREEN}Medusa server built successfully.${NC}"
else
  echo -e "  ${RED}Failed to build Medusa server.${NC}"
  exit 1
fi
echo ""
...
echo -e "${YELLOW}Starting Medusa server in production mode...${NC}"
npm run start &
SERVER_PID=$!
...
echo -e "${YELLOW}Building Next.js storefront...${NC}"
cd /workspace/Medusa-Bikeshop-Shipment/bikeshop-shipment-storefront
npm run build
if [ $? -eq 0 ]; then
  echo -e "  ${GREEN}Next.js storefront built successfully.${NC}"
else
  echo -e "  ${RED}Failed to build Next.js storefront.${NC}"
  exit 1
fi
echo ""

# Start the Next.js storefront in production mode
echo -e "${YELLOW}Starting Next.js storefront in production mode...${NC}"
npm run start -- -p 12001 --hostname 0.0.0.0 &
STOREFRONT_PID=$!
...
echo -e "${GREEN}=== Bikeshop Shipment Application is running in production mode! ===${NC}"
echo ""
